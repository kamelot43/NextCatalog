...
export const selectIsProductInComparison =
  (brand: string, productId: string) => (state: RootState) =>
    (state.comparison.idsByBrand[brand] ?? []).includes(productId);

export const selectIsComparisonLimitReached = (brand: string) => (state: RootState) => {
